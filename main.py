    ...
    try:
        ...
        if token:
            try:
                ...
                # Fetch instruments to enable Search/LTP
                smart_trader.fetch_instruments(kite)
                bot_active = True; login_state = "IDLE"
                gc.collect()
                print("✅ Session Generated Successfully & Instruments Fetched")
            except Exception as e:
                print(f"❌ Session Generation Error: {e}")
                login_state = "FAILED"; login_error_msg = str(e)
        else:
            if bot_active: login_state = "IDLE"
            else:
                print(f"❌ Auto-Login Failed: {error}")
                login_state = "FAILED"; login_error_msg = error
    except Exception as e:
        print(f"❌ Critical Session Error: {e}"); login_state = "FAILED"; login_error_msg = str(e)
            ...
            try:
                ...
                if bot_active:
                    try:
                        ...
                        err = str(e)
                        # Only disconnect on auth/network errors
                        if "Token is invalid" in err or "Network" in err or "access_token" in err:
                            print(f"⚠️ Connection Lost: {err}")
                            bot_active = False 

                # 2. Try Re-Login if Offline
                if not bot_active:
                    if login_state == "IDLE":
                        print("🔄 Monitor: System Offline. Initiating Auto-Login...")
                        run_auto_login_process()
                    elif login_state == "FAILED":
                        time.sleep(60); login_state = "IDLE"
...
@login_required
def logout():
    logout_user(); flash("Logged out successfully."); return redirect('/login')
...
def home():
    if bot_active:
        ...
        # Safe display name
        for t in trades: 
            try: t['symbol'] = smart_trader.get_display_name(t['symbol'])
            except: pass
...
def secure_login_page():
    if request.method == 'POST':
        if request.form.get('password') == config.ADMIN_PASSWORD: return redirect(kite.login_url())
        else: return render_template('secure_login.html', error="Invalid Password!")
...
def callback():
    ...
    if t:
        try:
            ...
            kite.set_access_token(data["access_token"])
            bot_active = True; smart_trader.fetch_instruments(kite); gc.collect()
            flash("✅ System Online")
...
def api_closed_trades():
    try:
        ...
        for t in trades: 
            try: t['symbol'] = smart_trader.get_display_name(t['symbol'])
            except: pass # Don't crash if display name fails
        return jsonify(trades)
    except Exception as e:
...
def api_search():
    try:
        ...
        return jsonify([]) # Return empty on error to prevent UI hang
...
def api_panic_exit():
    ...
    if strategy_manager.panic_exit_all(kite):
        flash("🚨 PANIC MODE EXECUTED."); return jsonify({"status": "success"})
    return jsonify({"status": "error"})
...
def api_import_trade():
    ...
    try:
        ...
        result = strategy_manager.import_past_trade(
            kite, final_sym, data['entry_time'], int(data['qty']), float(data['price']), 
            float(data['sl']), [float(t) for t in data['targets']], data.get('trailing_sl', 0), 
            data.get('sl_to_entry', 0), data.get('exit_multiplier', 1), data.get('target_controls')
        )
...
def api_simulate_trade():
    ...
    if not original_trade: return jsonify({"status": "error", "message": "Original trade not found"})
    try:
        symbol = original_trade['symbol']
        entry_time = original_trade['entry_time'].replace(' ', 'T')
        entry_price = original_trade['entry_price']; qty = original_trade['quantity']
        sl_points = float(data.get('sl_points', 0)); sl_price = entry_price - sl_points 
        targets = [float(t) for t in data.get('targets', [])]
        target_controls = data.get('target_controls'); trailing_sl = float(data.get('trailing_sl', 0))
        sl_to_entry = int(data.get('sl_to_entry', 0)); exit_multiplier = int(data.get('exit_multiplier', 1))

        result = strategy_manager.import_past_trade(
            kite, symbol, entry_time, qty, entry_price, sl_price, targets, trailing_sl, 
            sl_to_entry, exit_multiplier, target_controls, dry_run=True
        )
        return jsonify(result)
    except Exception as e: return jsonify({"status": "error", "message": str(e)})
...
# Run thread if Gunicorn (WERKZEUG_RUN_MAIN is typically NOT set in master, but standard run check)
